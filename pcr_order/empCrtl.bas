Attribute VB_Name = "empCrtl"
Sub importName(ByVal group As String)
   
    Dim eDb As New empDb
    Dim names As Variant
    Dim util As New testUtil
    Dim last_row As Long
    
    names = eDb.getEmpName()
    ...
    If Not util.isArrayEmpty(names) Then
        ...
        For i = LBound(names, 2) To UBound(names, 2)
            If compareName(names(0, i), group) Then
                empList.Cells(start + idx, 1).Value = names(0, i)
                idx = idx + 1
            End If
        Next i
    End If
